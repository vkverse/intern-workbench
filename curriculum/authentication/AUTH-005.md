...
id: AUTH-005
phase: AUTH
title: Protect Routes and Handle Auth Errors
type: BUILD
difficulty: advanced
...
# AUTH-005 — Protect Routes and Handle Auth Errors

## Objective

Complete a reviewable engineering exercise that demonstrates **protect routes and handle auth errors** and explain the decisions you made.

## Context

This is task 5 in the Authentication and Authorization phase. It builds one durable skill inside the same GitHub workflow used for product work: issue, branch, implementation, validation, pull request, review, and revision.

## What You Need to Learn

- The core concepts behind protect routes and handle auth errors.
...
1. Create or update the task artifact under `projects/full-stack/auth-005/`; do not scatter unrelated changes across the repository.
2. Add a short `README.md` in that task artifact containing the problem, your plan, commands used, and result.
3. Produce the smallest working example or analysis that demonstrates protect routes and handle auth errors.
...
- Primary workspace: `projects/full-stack/auth-005/`
- Evidence: `projects/full-stack/auth-005/README.md`
...
- [ ] The artifact directly demonstrates protect routes and handle auth errors.
...
From the repository root, enter `projects/full-stack/auth-005` and follow its README. If the task is documentation or investigation only, render/preview the Markdown and run the repository validation command instead.
...
1. Confirm the assigned issue title starts with `[AUTH-005]`.
...
`build: complete auth-005 protect routes and handle auth errors`
