...
id: AUTH-002
phase: AUTH
title: Register Users and Hash Passwords
type: BUILD
difficulty: advanced
...
Complete a reviewable engineering exercise that demonstrates **register users and hash passwords** and explain the decisions you made.

## Context

This is task 2 in the Authentication and Authorization phase. It builds one durable skill inside the same GitHub workflow used for product work: issue, branch, implementation, validation, pull request, review, and revision.

## What You Need to Learn

- The core concepts behind register users and hash passwords.
...
1. Create or update the task artifact under `projects/full-stack/auth-002/`; do not scatter unrelated changes across the repository.
2. Add a short `README.md` in that task artifact containing the problem, your plan, commands used, and result.
3. Produce the smallest working example or analysis that demonstrates register users and hash passwords.
...
- Primary workspace: `projects/full-stack/auth-002/`
- Evidence: `projects/full-stack/auth-002/README.md`
...
- [ ] The artifact directly demonstrates register users and hash passwords.
...
From the repository root, enter `projects/full-stack/auth-002` and follow its README. If the task is documentation or investigation only, render/preview the Markdown and run the repository validation command instead.
...
1. Confirm the assigned issue title starts with `[AUTH-002]`.
...
`task/<github-username>/auth-002-register-users-and-hash-passwords`

## Commit Message

`build: complete auth-002 register users and hash passwords`
