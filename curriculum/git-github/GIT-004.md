...
# GIT-004 — Restore Changes Safely

## Objective

Complete a reviewable engineering exercise that demonstrates **restore changes safely** and explain the decisions you made.

## Context

This is task 4 in the Git and GitHub phase. It builds one durable skill inside the same GitHub workflow used for product work: issue, branch, implementation, validation, pull request, review, and revision.

## What You Need to Learn

- The core concepts behind restore changes safely.
...
1. Create or update the task artifact under `practice/git-lab/git-004/`; do not scatter unrelated changes across the repository.
2. Add a short `README.md` in that task artifact containing the problem, your plan, commands used, and result.
3. Produce the smallest working example or analysis that demonstrates restore changes safely.
...
- Primary workspace: `practice/git-lab/git-004/`
- Evidence: `practice/git-lab/git-004/README.md`
...
- [ ] The artifact directly demonstrates restore changes safely.
...
From the repository root, enter `practice/git-lab/git-004` and follow its README. If the task is documentation or investigation only, render/preview the Markdown and run the repository validation command instead.
...
1. Confirm the assigned issue title starts with `[GIT-004]`.
...
`task/<github-username>/git-004-restore-changes-safely`

## Commit Message

`build: complete git-004 restore changes safely`
