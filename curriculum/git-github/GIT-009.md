...
id: GIT-009
phase: GIT
title: Create and Link Issues and Pull Requests
type: BUILD
difficulty: beginner
guidance: GUIDED
...
# GIT-009 — Create and Link Issues and Pull Requests

## Objective

Deliver a small change through a fork pull request.

## Context

This v0.5 exercise is part of the [starter route](../../tracks/starter.md). Read the [exercise contract](../../exercises/first-pr/README.md) before editing. Your mentor agrees the assignment and AI policy in an Issue.
...
1. Confirm origin points to your fork and upstream to the mentor repository.
2. Use this task branch to add a 100-word explanation of fork, clone, commit, push, and pull request in README.md.
3. Open a PR targeting the mentor main branch, link the assigned issue, and include actual check results and AI disclosure.

## Files / Folders

Run `npm run workbench -- start GIT-009 octocat` from the repository root after replacing `octocat` with your GitHub username. Work in `submissions/<github-username>/GIT-009/`. Keep shared curriculum and exercise contracts unchanged.
...
`task/<github-username>/git-009-first-pr`

Replace the placeholder; for example `task/octocat/git-009-first-pr`.

## Commit Message

`docs: submit GIT-009 evidence` for documentation; use `feat:` or `fix:` when implementing behavior.
...
- [ ] Verify: PR head is your fork and base is the mentor repository.
- [ ] Verify: The Issue number is distinct from GIT-009.
- [ ] Verify: Review feedback is answered on the same branch, not through a replacement PR.
