...
id: GIT-002
phase: GIT
title: Create a Repository and Inspect Status
type: BUILD
difficulty: beginner
guidance: GUIDED
...
# GIT-002 — Create a Repository and Inspect Status

## Objective

Complete a reviewable engineering exercise that demonstrates **create a repository and inspect status** and explain the decisions you made.

## Context

This is task 2 in the Git and GitHub phase. It builds one durable skill inside the same GitHub workflow used for product work: issue, branch, implementation, validation, pull request, review, and revision.

## What You Need to Learn

- The core concepts behind create a repository and inspect status.
...
1. Create or update the task artifact under `practice/git-lab/git-002/`; do not scatter unrelated changes across the repository.
2. Add a short `README.md` in that task artifact containing the problem, your plan, commands used, and result.
3. Produce the smallest working example or analysis that demonstrates create a repository and inspect status.
...
- Primary workspace: `practice/git-lab/git-002/`
- Evidence: `practice/git-lab/git-002/README.md`
...
- [ ] The artifact directly demonstrates create a repository and inspect status.
...
From the repository root, enter `practice/git-lab/git-002` and follow its README. If the task is documentation or investigation only, render/preview the Markdown and run the repository validation command instead.
...
1. Confirm the assigned issue title starts with `[GIT-002]`.
...
`task/<github-username>/git-002-create-a-repository-and-inspect-status`

## Commit Message

`build: complete git-002 create a repository and inspect status`
