...
id: DB-008
phase: DB
title: Install PostgreSQL and Connect an Application
type: BUILD
...
# DB-008 — Install PostgreSQL and Connect an Application

## Objective

Complete a reviewable engineering exercise that demonstrates **install postgresql and connect an application** and explain the decisions you made.

## Context

This is task 8 in the PostgreSQL and Databases phase. It builds one durable skill inside the same GitHub workflow used for product work: issue, branch, implementation, validation, pull request, review, and revision.

## What You Need to Learn

- The core concepts behind install postgresql and connect an application.
...
1. Create or update the task artifact under `projects/full-stack/database/db-008/`; do not scatter unrelated changes across the repository.
2. Add a short `README.md` in that task artifact containing the problem, your plan, commands used, and result.
3. Produce the smallest working example or analysis that demonstrates install postgresql and connect an application.
...
- Primary workspace: `projects/full-stack/database/db-008/`
- Evidence: `projects/full-stack/database/db-008/README.md`
...
Use migrations and seed data; never put credentials in the repository.
...
- [ ] The artifact directly demonstrates install postgresql and connect an application.
...
From the repository root, enter `projects/full-stack/database/db-008` and follow its README. If the task is documentation or investigation only, render/preview the Markdown and run the repository validation command instead.
...
1. Confirm the assigned issue title starts with `[DB-008]`.
...
`task/<github-username>/db-008-install-postgresql-and-connect-an-applicat`

## Commit Message

`build: complete db-008 install postgresql and connect an application`
