...
# DB-007 — Indexes and Query Performance

## Objective

Complete a reviewable engineering exercise that demonstrates **indexes and query performance** and explain the decisions you made.

## Context

This is task 7 in the PostgreSQL and Databases phase. It builds one durable skill inside the same GitHub workflow used for product work: issue, branch, implementation, validation, pull request, review, and revision.

## What You Need to Learn

- The core concepts behind indexes and query performance.
...
1. Create or update the task artifact under `projects/full-stack/database/db-007/`; do not scatter unrelated changes across the repository.
2. Add a short `README.md` in that task artifact containing the problem, your plan, commands used, and result.
3. Produce the smallest working example or analysis that demonstrates indexes and query performance.
...
- Primary workspace: `projects/full-stack/database/db-007/`
- Evidence: `projects/full-stack/database/db-007/README.md`
...
- [ ] The artifact directly demonstrates indexes and query performance.
...
From the repository root, enter `projects/full-stack/database/db-007` and follow its README. If the task is documentation or investigation only, render/preview the Markdown and run the repository validation command instead.
...
1. Confirm the assigned issue title starts with `[DB-007]`.
...
`task/<github-username>/db-007-indexes-and-query-performance`

## Commit Message

`build: complete db-007 indexes and query performance`
