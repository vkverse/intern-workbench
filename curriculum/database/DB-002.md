...
id: DB-002
phase: DB
title: Query with SELECT and WHERE
type: BUILD
...
# DB-002 — Query with SELECT and WHERE

## Objective

Complete a reviewable engineering exercise that demonstrates **query with select and where** and explain the decisions you made.

## Context

This is task 2 in the PostgreSQL and Databases phase. It builds one durable skill inside the same GitHub workflow used for product work: issue, branch, implementation, validation, pull request, review, and revision.

## What You Need to Learn

- The core concepts behind query with select and where.
...
1. Create or update the task artifact under `projects/full-stack/database/db-002/`; do not scatter unrelated changes across the repository.
2. Add a short `README.md` in that task artifact containing the problem, your plan, commands used, and result.
3. Produce the smallest working example or analysis that demonstrates query with select and where.
...
- Primary workspace: `projects/full-stack/database/db-002/`
- Evidence: `projects/full-stack/database/db-002/README.md`
...
- [ ] The artifact directly demonstrates query with select and where.
...
From the repository root, enter `projects/full-stack/database/db-002` and follow its README. If the task is documentation or investigation only, render/preview the Markdown and run the repository validation command instead.
...
1. Confirm the assigned issue title starts with `[DB-002]`.
...
`task/<github-username>/db-002-query-with-select-and-where`

## Commit Message

`build: complete db-002 query with select and where`
