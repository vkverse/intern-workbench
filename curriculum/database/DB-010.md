...
id: DB-010
phase: DB
title: Design and Build a Database-Backed API
type: BUILD
...
# DB-010 — Design and Build a Database-Backed API

## Objective

Complete a reviewable engineering exercise that demonstrates **design and build a database-backed api** and explain the decisions you made.

## Context

This is task 10 in the PostgreSQL and Databases phase. It builds one durable skill inside the same GitHub workflow used for product work: issue, branch, implementation, validation, pull request, review, and revision.

## What You Need to Learn

- The core concepts behind design and build a database-backed api.
...
1. Create or update the task artifact under `projects/full-stack/database/db-010/`; do not scatter unrelated changes across the repository.
2. Add a short `README.md` in that task artifact containing the problem, your plan, commands used, and result.
3. Produce the smallest working example or analysis that demonstrates design and build a database-backed api.
...
- Primary workspace: `projects/full-stack/database/db-010/`
- Evidence: `projects/full-stack/database/db-010/README.md`
...
- [ ] The artifact directly demonstrates design and build a database-backed api.
...
From the repository root, enter `projects/full-stack/database/db-010` and follow its README. If the task is documentation or investigation only, render/preview the Markdown and run the repository validation command instead.
...
1. Confirm the assigned issue title starts with `[DB-010]`.
...
`task/<github-username>/db-010-design-and-build-a-database-backed-api`

## Commit Message

`build: complete db-010 design and build a database-backed api`
