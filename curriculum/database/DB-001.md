...
id: DB-001
phase: DB
title: Database, Table, Row, and Key Fundamentals
type: BUILD
...
# DB-001 — Database, Table, Row, and Key Fundamentals

## Objective

Complete a reviewable engineering exercise that demonstrates **database, table, row, and key fundamentals** and explain the decisions you made.

## Context

This is task 1 in the PostgreSQL and Databases phase. It builds one durable skill inside the same GitHub workflow used for product work: issue, branch, implementation, validation, pull request, review, and revision.

## What You Need to Learn

- The core concepts behind database, table, row, and key fundamentals.
...
1. Create or update the task artifact under `projects/full-stack/database/db-001/`; do not scatter unrelated changes across the repository.
2. Add a short `README.md` in that task artifact containing the problem, your plan, commands used, and result.
3. Produce the smallest working example or analysis that demonstrates database, table, row, and key fundamentals.
...
- Primary workspace: `projects/full-stack/database/db-001/`
- Evidence: `projects/full-stack/database/db-001/README.md`
...
- [ ] The artifact directly demonstrates database, table, row, and key fundamentals.
...
From the repository root, enter `projects/full-stack/database/db-001` and follow its README. If the task is documentation or investigation only, render/preview the Markdown and run the repository validation command instead.
...
1. Confirm the assigned issue title starts with `[DB-001]`.
...
`task/<github-username>/db-001-database-table-row-and-key-fundamentals`

## Commit Message

`build: complete db-001 database, table, row, and key fundamentals`
