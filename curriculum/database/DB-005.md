...
# DB-005 — Join and Aggregate Data

## Objective

Complete a reviewable engineering exercise that demonstrates **join and aggregate data** and explain the decisions you made.

## Context

This is task 5 in the PostgreSQL and Databases phase. It builds one durable skill inside the same GitHub workflow used for product work: issue, branch, implementation, validation, pull request, review, and revision.

## What You Need to Learn

- The core concepts behind join and aggregate data.
...
1. Create or update the task artifact under `projects/full-stack/database/db-005/`; do not scatter unrelated changes across the repository.
2. Add a short `README.md` in that task artifact containing the problem, your plan, commands used, and result.
3. Produce the smallest working example or analysis that demonstrates join and aggregate data.
...
- Primary workspace: `projects/full-stack/database/db-005/`
- Evidence: `projects/full-stack/database/db-005/README.md`
...
- [ ] The artifact directly demonstrates join and aggregate data.
...
From the repository root, enter `projects/full-stack/database/db-005` and follow its README. If the task is documentation or investigation only, render/preview the Markdown and run the repository validation command instead.
...
1. Confirm the assigned issue title starts with `[DB-005]`.
...
`task/<github-username>/db-005-join-and-aggregate-data`

## Commit Message

`build: complete db-005 join and aggregate data`
