...
id: DB-004
phase: DB
title: Model Relationships and Foreign Keys
type: BUILD
...
# DB-004 — Model Relationships and Foreign Keys

## Objective

Complete a reviewable engineering exercise that demonstrates **model relationships and foreign keys** and explain the decisions you made.

## Context

This is task 4 in the PostgreSQL and Databases phase. It builds one durable skill inside the same GitHub workflow used for product work: issue, branch, implementation, validation, pull request, review, and revision.

## What You Need to Learn

- The core concepts behind model relationships and foreign keys.
...
1. Create or update the task artifact under `projects/full-stack/database/db-004/`; do not scatter unrelated changes across the repository.
2. Add a short `README.md` in that task artifact containing the problem, your plan, commands used, and result.
3. Produce the smallest working example or analysis that demonstrates model relationships and foreign keys.
...
- Primary workspace: `projects/full-stack/database/db-004/`
- Evidence: `projects/full-stack/database/db-004/README.md`
...
- [ ] The artifact directly demonstrates model relationships and foreign keys.
...
From the repository root, enter `projects/full-stack/database/db-004` and follow its README. If the task is documentation or investigation only, render/preview the Markdown and run the repository validation command instead.
...
1. Confirm the assigned issue title starts with `[DB-004]`.
...
`task/<github-username>/db-004-model-relationships-and-foreign-keys`

## Commit Message

`build: complete db-004 model relationships and foreign keys`
