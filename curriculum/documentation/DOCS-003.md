...
# DOCS-003 — Document an API

## Objective

Complete a reviewable engineering exercise that demonstrates **document an api** and explain the decisions you made.

## Context

This is task 3 in the Documentation phase. It builds one durable skill inside the same GitHub workflow used for product work: issue, branch, implementation, validation, pull request, review, and revision.

## What You Need to Learn

- The core concepts behind document an api.
...
1. Create or update the task artifact under `docs/practice/docs-003/`; do not scatter unrelated changes across the repository.
2. Add a short `README.md` in that task artifact containing the problem, your plan, commands used, and result.
3. Produce the smallest working example or analysis that demonstrates document an api.
...
- Primary workspace: `docs/practice/docs-003/`
- Evidence: `docs/practice/docs-003/README.md`
...
- [ ] The artifact directly demonstrates document an api.
...
From the repository root, enter `docs/practice/docs-003` and follow its README. If the task is documentation or investigation only, render/preview the Markdown and run the repository validation command instead.
...
1. Confirm the assigned issue title starts with `[DOCS-003]`.
...
`document: complete docs-003 document an api`
