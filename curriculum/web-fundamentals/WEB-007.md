...
id: WEB-007
phase: WEB
title: Rendering, Static Generation, and Caching
type: INVESTIGATE
...
# WEB-007 — Rendering, Static Generation, and Caching

## Objective

Complete a reviewable engineering exercise that demonstrates **rendering, static generation, and caching** and explain the decisions you made.

## Context

This is task 7 in the Web Fundamentals phase. It builds one durable skill inside the same GitHub workflow used for product work: issue, branch, implementation, validation, pull request, review, and revision.

## What You Need to Learn

- The core concepts behind rendering, static generation, and caching.
...
1. Create or update the task artifact under `evidence/web/web-007/`; do not scatter unrelated changes across the repository.
2. Add a short `README.md` in that task artifact containing the problem, your plan, commands used, and result.
3. Produce the smallest working example or analysis that demonstrates rendering, static generation, and caching.
...
- Primary workspace: `evidence/web/web-007/`
- Evidence: `evidence/web/web-007/README.md`
...
Capture real request/response evidence and redact credentials or cookies.
...
- [ ] The artifact directly demonstrates rendering, static generation, and caching.
...
From the repository root, enter `evidence/web/web-007` and follow its README. If the task is documentation or investigation only, render/preview the Markdown and run the repository validation command instead.
...
1. Confirm the assigned issue title starts with `[WEB-007]`.
...
`task/<github-username>/web-007-rendering-static-generation-and-caching`

## Commit Message

`investigate: complete web-007 rendering, static generation, and caching`
