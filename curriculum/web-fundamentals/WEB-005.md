...
id: WEB-005
phase: WEB
title: CORS and Browser Security Boundaries
type: INVESTIGATE
...
# WEB-005 — CORS and Browser Security Boundaries

## Objective

Complete a reviewable engineering exercise that demonstrates **cors and browser security boundaries** and explain the decisions you made.

## Context

This is task 5 in the Web Fundamentals phase. It builds one durable skill inside the same GitHub workflow used for product work: issue, branch, implementation, validation, pull request, review, and revision.

## What You Need to Learn

- The core concepts behind cors and browser security boundaries.
...
1. Create or update the task artifact under `evidence/web/web-005/`; do not scatter unrelated changes across the repository.
2. Add a short `README.md` in that task artifact containing the problem, your plan, commands used, and result.
3. Produce the smallest working example or analysis that demonstrates cors and browser security boundaries.
...
- Primary workspace: `evidence/web/web-005/`
- Evidence: `evidence/web/web-005/README.md`
...
- [ ] The artifact directly demonstrates cors and browser security boundaries.
...
From the repository root, enter `evidence/web/web-005` and follow its README. If the task is documentation or investigation only, render/preview the Markdown and run the repository validation command instead.
...
1. Confirm the assigned issue title starts with `[WEB-005]`.
...
`task/<github-username>/web-005-cors-and-browser-security-boundaries`

## Commit Message

`investigate: complete web-005 cors and browser security boundaries`
