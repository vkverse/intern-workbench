...
# WEB-003 — Methods, Status Codes, and Headers

## Objective

Complete a reviewable engineering exercise that demonstrates **methods, status codes, and headers** and explain the decisions you made.

## Context

This is task 3 in the Web Fundamentals phase. It builds one durable skill inside the same GitHub workflow used for product work: issue, branch, implementation, validation, pull request, review, and revision.

## What You Need to Learn

- The core concepts behind methods, status codes, and headers.
...
1. Create or update the task artifact under `evidence/web/web-003/`; do not scatter unrelated changes across the repository.
2. Add a short `README.md` in that task artifact containing the problem, your plan, commands used, and result.
3. Produce the smallest working example or analysis that demonstrates methods, status codes, and headers.
...
- Primary workspace: `evidence/web/web-003/`
- Evidence: `evidence/web/web-003/README.md`
...
- [ ] The artifact directly demonstrates methods, status codes, and headers.
...
From the repository root, enter `evidence/web/web-003` and follow its README. If the task is documentation or investigation only, render/preview the Markdown and run the repository validation command instead.
...
1. Confirm the assigned issue title starts with `[WEB-003]`.
...
`task/<github-username>/web-003-methods-status-codes-and-headers`

## Commit Message

`investigate: complete web-003 methods, status codes, and headers`
