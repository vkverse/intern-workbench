...
id: WEB-008
phase: WEB
title: Build and Debug an HTTP Integration
type: INVESTIGATE
...
# WEB-008 — Build and Debug an HTTP Integration

## Objective

Complete a reviewable engineering exercise that demonstrates **build and debug an http integration** and explain the decisions you made.

## Context

This is task 8 in the Web Fundamentals phase. It builds one durable skill inside the same GitHub workflow used for product work: issue, branch, implementation, validation, pull request, review, and revision.

## What You Need to Learn

- The core concepts behind build and debug an http integration.
...
1. Create or update the task artifact under `evidence/web/web-008/`; do not scatter unrelated changes across the repository.
2. Add a short `README.md` in that task artifact containing the problem, your plan, commands used, and result.
3. Produce the smallest working example or analysis that demonstrates build and debug an http integration.
...
- Primary workspace: `evidence/web/web-008/`
- Evidence: `evidence/web/web-008/README.md`
...
- [ ] The artifact directly demonstrates build and debug an http integration.
...
From the repository root, enter `evidence/web/web-008` and follow its README. If the task is documentation or investigation only, render/preview the Markdown and run the repository validation command instead.
...
1. Confirm the assigned issue title starts with `[WEB-008]`.
...
`task/<github-username>/web-008-build-and-debug-an-http-integration`

## Commit Message

`investigate: complete web-008 build and debug an http integration`
