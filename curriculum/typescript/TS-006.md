...
id: TS-006
phase: TS
title: Narrow Unknown Values Safely
type: REFACTOR
...
# TS-006 — Narrow Unknown Values Safely

## Objective

Complete a reviewable engineering exercise that demonstrates **narrow unknown values safely** and explain the decisions you made.

## Context

This is task 6 in the TypeScript phase. It builds one durable skill inside the same GitHub workflow used for product work: issue, branch, implementation, validation, pull request, review, and revision.

## What You Need to Learn

- The core concepts behind narrow unknown values safely.
...
1. Create or update the task artifact under `projects/intermediate/typed-app/ts-006/`; do not scatter unrelated changes across the repository.
2. Add a short `README.md` in that task artifact containing the problem, your plan, commands used, and result.
3. Produce the smallest working example or analysis that demonstrates narrow unknown values safely.
...
- Primary workspace: `projects/intermediate/typed-app/ts-006/`
- Evidence: `projects/intermediate/typed-app/ts-006/README.md`
...
- [ ] The artifact directly demonstrates narrow unknown values safely.
...
From the repository root, enter `projects/intermediate/typed-app/ts-006` and follow its README. If the task is documentation or investigation only, render/preview the Markdown and run the repository validation command instead.
...
1. Confirm the assigned issue title starts with `[TS-006]`.
...
`task/<github-username>/ts-006-narrow-unknown-values-safely`

## Commit Message

`refactor: complete ts-006 narrow unknown values safely`
