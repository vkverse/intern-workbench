...
# TS-001 — Types and Inference

## Objective

Complete a reviewable engineering exercise that demonstrates **types and inference** and explain the decisions you made.

## Context

This is task 1 in the TypeScript phase. It builds one durable skill inside the same GitHub workflow used for product work: issue, branch, implementation, validation, pull request, review, and revision.

## What You Need to Learn

- The core concepts behind types and inference.
...
1. Create or update the task artifact under `projects/intermediate/typed-app/ts-001/`; do not scatter unrelated changes across the repository.
2. Add a short `README.md` in that task artifact containing the problem, your plan, commands used, and result.
3. Produce the smallest working example or analysis that demonstrates types and inference.
...
- Primary workspace: `projects/intermediate/typed-app/ts-001/`
- Evidence: `projects/intermediate/typed-app/ts-001/README.md`
...
- [ ] The artifact directly demonstrates types and inference.
...
From the repository root, enter `projects/intermediate/typed-app/ts-001` and follow its README. If the task is documentation or investigation only, render/preview the Markdown and run the repository validation command instead.
...
1. Confirm the assigned issue title starts with `[TS-001]`.
...
`task/<github-username>/ts-001-types-and-inference`

## Commit Message

`refactor: complete ts-001 types and inference`
