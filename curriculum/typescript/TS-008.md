...
id: TS-008
phase: TS
title: Migrate and Debug JavaScript as TypeScript
type: REFACTOR
...
# TS-008 — Migrate and Debug JavaScript as TypeScript

## Objective

Complete a reviewable engineering exercise that demonstrates **migrate and debug javascript as typescript** and explain the decisions you made.

## Context

This is task 8 in the TypeScript phase. It builds one durable skill inside the same GitHub workflow used for product work: issue, branch, implementation, validation, pull request, review, and revision.

## What You Need to Learn

- The core concepts behind migrate and debug javascript as typescript.
...
1. Create or update the task artifact under `projects/intermediate/typed-app/ts-008/`; do not scatter unrelated changes across the repository.
2. Add a short `README.md` in that task artifact containing the problem, your plan, commands used, and result.
3. Produce the smallest working example or analysis that demonstrates migrate and debug javascript as typescript.
...
- Primary workspace: `projects/intermediate/typed-app/ts-008/`
- Evidence: `projects/intermediate/typed-app/ts-008/README.md`
...
Do not use any or type assertions to silence errors without written justification.
...
- [ ] The artifact directly demonstrates migrate and debug javascript as typescript.
...
From the repository root, enter `projects/intermediate/typed-app/ts-008` and follow its README. If the task is documentation or investigation only, render/preview the Markdown and run the repository validation command instead.
...
1. Confirm the assigned issue title starts with `[TS-008]`.
...
`task/<github-username>/ts-008-migrate-and-debug-javascript-as-typescript`

## Commit Message

`refactor: complete ts-008 migrate and debug javascript as typescript`
