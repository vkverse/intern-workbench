...
Complete a reviewable engineering exercise that demonstrates **middleware and request lifecycle** and explain the decisions you made.

## Context

This is task 4 in the Node.js phase. It builds one durable skill inside the same GitHub workflow used for product work: issue, branch, implementation, validation, pull request, review, and revision.

## What You Need to Learn

- The core concepts behind middleware and request lifecycle.
...
1. Create or update the task artifact under `projects/full-stack/api/node-004/`; do not scatter unrelated changes across the repository.
2. Add a short `README.md` in that task artifact containing the problem, your plan, commands used, and result.
3. Produce the smallest working example or analysis that demonstrates middleware and request lifecycle.
...
- Primary workspace: `projects/full-stack/api/node-004/`
- Evidence: `projects/full-stack/api/node-004/README.md`
...
- [ ] The artifact directly demonstrates middleware and request lifecycle.
...
From the repository root, enter `projects/full-stack/api/node-004` and follow its README. If the task is documentation or investigation only, render/preview the Markdown and run the repository validation command instead.
...
1. Confirm the assigned issue title starts with `[NODE-004]`.
...
`task/<github-username>/node-004-middleware-and-request-lifecycle`

## Commit Message

`build: complete node-004 middleware and request lifecycle`
