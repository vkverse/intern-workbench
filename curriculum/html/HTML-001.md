...
id: HTML-001
phase: HTML
title: Create a Semantic HTML Document
type: BUILD
difficulty: beginner
guidance: GUIDED
...
# HTML-001 — Create a Semantic HTML Document

## Objective

Build a semantic community profile page.

## Context

This v0.5 exercise is part of the [starter route](../../tracks/starter.md). Read the [exercise contract](../../exercises/profile/README.md) before editing. Your mentor agrees the assignment and AI policy in an Issue.
...
1. Create index.html with a doctype, html lang, UTF-8 charset, viewport, and descriptive title.
2. Add one h1, a header, nav, main, two titled sections, and footer. Use a fictional profile with About and Contact links targeting real section IDs.
3. Open the page in a browser; navigate every link using only the keyboard.

## Files / Folders

Run `npm run workbench -- start HTML-001 octocat` from the repository root after replacing `octocat` with your GitHub username. Work in `submissions/<github-username>/HTML-001/`. Keep shared curriculum and exercise contracts unchanged.
...
`task/<github-username>/html-001-profile`

Replace the placeholder; for example `task/octocat/html-001-profile`.

## Commit Message

`docs: submit HTML-001 evidence` for documentation; use `feat:` or `fix:` when implementing behavior.
...
- [ ] Verify: Document landmarks and heading hierarchy match the content.
- [ ] Verify: Every in-page link reaches an existing section.
- [ ] Verify: No personal contact data or external service is required.
