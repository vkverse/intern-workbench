...
id: SEC-004
phase: SEC
title: Find and Fix SQL Injection
type: INVESTIGATE
difficulty: advanced
...
# SEC-004 — Find and Fix SQL Injection

## Objective

Complete a reviewable engineering exercise that demonstrates **find and fix sql injection** and explain the decisions you made.

## Context

This is task 4 in the Security phase. It builds one durable skill inside the same GitHub workflow used for product work: issue, branch, implementation, validation, pull request, review, and revision.

## What You Need to Learn

- The core concepts behind find and fix sql injection.
...
1. Create or update the task artifact under `projects/advanced/security-lab/sec-004/`; do not scatter unrelated changes across the repository.
2. Add a short `README.md` in that task artifact containing the problem, your plan, commands used, and result.
3. Produce the smallest working example or analysis that demonstrates find and fix sql injection.
...
- Primary workspace: `projects/advanced/security-lab/sec-004/`
- Evidence: `projects/advanced/security-lab/sec-004/README.md`
...
- [ ] The artifact directly demonstrates find and fix sql injection.
...
From the repository root, enter `projects/advanced/security-lab/sec-004` and follow its README. If the task is documentation or investigation only, render/preview the Markdown and run the repository validation command instead.
...
1. Confirm the assigned issue title starts with `[SEC-004]`.
...
`task/<github-username>/sec-004-find-and-fix-sql-injection`

## Commit Message

`investigate: complete sec-004 find and fix sql injection`
