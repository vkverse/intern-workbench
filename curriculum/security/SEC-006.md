...
id: SEC-006
phase: SEC
title: Manage Secrets and Environment Configuration
type: BUILD
...
# SEC-006 — Manage Secrets and Environment Configuration
...
This v0.5 exercise is part of the [starter route](../../tracks/starter.md). Read the [exercise contract](../../exercises/redaction/README.md) before editing. Your mentor agrees the assignment and AI policy in an Issue.
...
Run `npm run workbench -- start SEC-006 octocat` from the repository root after replacing `octocat` with your GitHub username. Work in `submissions/<github-username>/SEC-006/`. Keep shared curriculum and exercise contracts unchanged.
...
`task/<github-username>/sec-006-redaction`

Replace the placeholder; for example `task/octocat/sec-006-redaction`.

## Commit Message

`docs: submit SEC-006 evidence` for documentation; use `feat:` or `fix:` when implementing behavior.
...
- [ ] Verify: Nested objects, arrays, uppercase key names, and null values are handled.
- [ ] Verify: Input remains unchanged and safe values are preserved.
- [ ] Verify: No actual credential is used in the exercise or evidence.
