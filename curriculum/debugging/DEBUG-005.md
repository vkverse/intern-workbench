...
id: DEBUG-005
phase: DEBUG
title: Diagnose Backend and Database Failures
type: DEBUG
difficulty: advanced
...
Complete a reviewable engineering exercise that demonstrates **diagnose backend and database failures** and explain the decisions you made.

## Context

This is task 5 in the Debugging phase. It builds one durable skill inside the same GitHub workflow used for product work: issue, branch, implementation, validation, pull request, review, and revision.

## What You Need to Learn

- The core concepts behind diagnose backend and database failures.
...
1. Create or update the task artifact under `projects/advanced/incident-lab/debug-005/`; do not scatter unrelated changes across the repository.
2. Add a short `README.md` in that task artifact containing the problem, your plan, commands used, and result.
3. Produce the smallest working example or analysis that demonstrates diagnose backend and database failures.
...
- Primary workspace: `projects/advanced/incident-lab/debug-005/`
- Evidence: `projects/advanced/incident-lab/debug-005/README.md`
...
- [ ] The artifact directly demonstrates diagnose backend and database failures.
...
From the repository root, enter `projects/advanced/incident-lab/debug-005` and follow its README. If the task is documentation or investigation only, render/preview the Markdown and run the repository validation command instead.
...
1. Confirm the assigned issue title starts with `[DEBUG-005]`.
...
`task/<github-username>/debug-005-diagnose-backend-and-database-failures`

## Commit Message

`debug: complete debug-005 diagnose backend and database failures`
