...
id: API-007
phase: API
title: Test and Rate-Limit an API
type: BUILD
...
# API-007 — Test and Rate-Limit an API

## Objective

Complete a reviewable engineering exercise that demonstrates **test and rate-limit an api** and explain the decisions you made.

## Context

This is task 7 in the REST APIs phase. It builds one durable skill inside the same GitHub workflow used for product work: issue, branch, implementation, validation, pull request, review, and revision.

## What You Need to Learn

- The core concepts behind test and rate-limit an api.
...
1. Create or update the task artifact under `projects/full-stack/api/api-007/`; do not scatter unrelated changes across the repository.
2. Add a short `README.md` in that task artifact containing the problem, your plan, commands used, and result.
3. Produce the smallest working example or analysis that demonstrates test and rate-limit an api.
...
- Primary workspace: `projects/full-stack/api/api-007/`
- Evidence: `projects/full-stack/api/api-007/README.md`
...
- [ ] The artifact directly demonstrates test and rate-limit an api.
...
From the repository root, enter `projects/full-stack/api/api-007` and follow its README. If the task is documentation or investigation only, render/preview the Markdown and run the repository validation command instead.
...
1. Confirm the assigned issue title starts with `[API-007]`.
...
`task/<github-username>/api-007-test-and-rate-limit-an-api`

## Commit Message

`build: complete api-007 test and rate-limit an api`
