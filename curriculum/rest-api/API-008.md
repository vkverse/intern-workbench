...
id: API-008
phase: API
title: Build and Refactor a CRUD API
type: BUILD
...
# API-008 — Build and Refactor a CRUD API

## Objective

Complete a reviewable engineering exercise that demonstrates **build and refactor a crud api** and explain the decisions you made.

## Context

This is task 8 in the REST APIs phase. It builds one durable skill inside the same GitHub workflow used for product work: issue, branch, implementation, validation, pull request, review, and revision.

## What You Need to Learn

- The core concepts behind build and refactor a crud api.
...
1. Create or update the task artifact under `projects/full-stack/api/api-008/`; do not scatter unrelated changes across the repository.
2. Add a short `README.md` in that task artifact containing the problem, your plan, commands used, and result.
3. Produce the smallest working example or analysis that demonstrates build and refactor a crud api.
...
- Primary workspace: `projects/full-stack/api/api-008/`
- Evidence: `projects/full-stack/api/api-008/README.md`
...
- [ ] The artifact directly demonstrates build and refactor a crud api.
...
From the repository root, enter `projects/full-stack/api/api-008` and follow its README. If the task is documentation or investigation only, render/preview the Markdown and run the repository validation command instead.
...
1. Confirm the assigned issue title starts with `[API-008]`.
...
`task/<github-username>/api-008-build-and-refactor-a-crud-api`

## Commit Message

`build: complete api-008 build and refactor a crud api`
