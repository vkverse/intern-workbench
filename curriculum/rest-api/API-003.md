...
id: API-003
phase: API
title: Validate Requests and Design Responses
type: BUILD
...
# API-003 — Validate Requests and Design Responses

## Objective

Complete a reviewable engineering exercise that demonstrates **validate requests and design responses** and explain the decisions you made.

## Context

This is task 3 in the REST APIs phase. It builds one durable skill inside the same GitHub workflow used for product work: issue, branch, implementation, validation, pull request, review, and revision.

## What You Need to Learn

- The core concepts behind validate requests and design responses.
...
1. Create or update the task artifact under `projects/full-stack/api/api-003/`; do not scatter unrelated changes across the repository.
2. Add a short `README.md` in that task artifact containing the problem, your plan, commands used, and result.
3. Produce the smallest working example or analysis that demonstrates validate requests and design responses.
...
- Primary workspace: `projects/full-stack/api/api-003/`
- Evidence: `projects/full-stack/api/api-003/README.md`
...
Follow the documented API contract and return safe, consistent errors.
...
- [ ] The artifact directly demonstrates validate requests and design responses.
...
From the repository root, enter `projects/full-stack/api/api-003` and follow its README. If the task is documentation or investigation only, render/preview the Markdown and run the repository validation command instead.
...
1. Confirm the assigned issue title starts with `[API-003]`.
...
`task/<github-username>/api-003-validate-requests-and-design-responses`

## Commit Message

`build: complete api-003 validate requests and design responses`
