...
id: API-004
phase: API
title: Handle Errors and Status Codes
type: BUILD
...
# API-004 — Handle Errors and Status Codes

## Objective

Complete a reviewable engineering exercise that demonstrates **handle errors and status codes** and explain the decisions you made.

## Context

This is task 4 in the REST APIs phase. It builds one durable skill inside the same GitHub workflow used for product work: issue, branch, implementation, validation, pull request, review, and revision.

## What You Need to Learn

- The core concepts behind handle errors and status codes.
...
1. Create or update the task artifact under `projects/full-stack/api/api-004/`; do not scatter unrelated changes across the repository.
2. Add a short `README.md` in that task artifact containing the problem, your plan, commands used, and result.
3. Produce the smallest working example or analysis that demonstrates handle errors and status codes.
...
- Primary workspace: `projects/full-stack/api/api-004/`
- Evidence: `projects/full-stack/api/api-004/README.md`
...
- [ ] The artifact directly demonstrates handle errors and status codes.
...
From the repository root, enter `projects/full-stack/api/api-004` and follow its README. If the task is documentation or investigation only, render/preview the Markdown and run the repository validation command instead.
...
1. Confirm the assigned issue title starts with `[API-004]`.
...
`task/<github-username>/api-004-handle-errors-and-status-codes`

## Commit Message

`build: complete api-004 handle errors and status codes`
