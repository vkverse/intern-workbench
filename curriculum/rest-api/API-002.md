...
id: API-002
phase: API
title: Separate Routes and Controllers
type: BUILD
...
# API-002 — Separate Routes and Controllers

## Objective

Complete a reviewable engineering exercise that demonstrates **separate routes and controllers** and explain the decisions you made.

## Context

This is task 2 in the REST APIs phase. It builds one durable skill inside the same GitHub workflow used for product work: issue, branch, implementation, validation, pull request, review, and revision.

## What You Need to Learn

- The core concepts behind separate routes and controllers.
...
1. Create or update the task artifact under `projects/full-stack/api/api-002/`; do not scatter unrelated changes across the repository.
2. Add a short `README.md` in that task artifact containing the problem, your plan, commands used, and result.
3. Produce the smallest working example or analysis that demonstrates separate routes and controllers.
...
- Primary workspace: `projects/full-stack/api/api-002/`
- Evidence: `projects/full-stack/api/api-002/README.md`
...
- [ ] The artifact directly demonstrates separate routes and controllers.
...
From the repository root, enter `projects/full-stack/api/api-002` and follow its README. If the task is documentation or investigation only, render/preview the Markdown and run the repository validation command instead.
...
1. Confirm the assigned issue title starts with `[API-002]`.
...
`task/<github-username>/api-002-separate-routes-and-controllers`

## Commit Message

`build: complete api-002 separate routes and controllers`
