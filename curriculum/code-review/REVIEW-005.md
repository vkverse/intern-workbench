...
id: REVIEW-005
phase: REVIEW
title: Write Useful, Respectful Review Comments
type: REVIEW
...
# REVIEW-005 — Write Useful, Respectful Review Comments

## Objective

Review a flawed cart implementation without changing it.

## Context

This v0.5 exercise is part of the [starter route](../../tracks/starter.md). Read the [exercise contract](../../exercises/review/README.md) before editing. Your mentor agrees the assignment and AI policy in an Issue.
...
1. Read exercises/review/fixture.mjs and its contract. Write three comments in README.md with file/line, reproducible input, expected/actual output, and impact.
2. Distinguish a correctness blocker from a suggestion; propose a test for each blocker.
3. Include one respectful question about rounding or validation requirements instead of assuming product policy.

## Files / Folders

Run `npm run workbench -- start REVIEW-005 octocat` from the repository root after replacing `octocat` with your GitHub username. Work in `submissions/<github-username>/REVIEW-005/`. Keep shared curriculum and exercise contracts unchanged.
...
`task/<github-username>/review-005-review`

Replace the placeholder; for example `task/octocat/review-005-review`.

## Commit Message

`docs: submit REVIEW-005 evidence` for documentation; use `feat:` or `fix:` when implementing behavior.
...
- [ ] Verify: Comments identify defects using concrete evidence.
- [ ] Verify: The review does not replace the implementation.
- [ ] Verify: Tone addresses the code and requirement rather than the author.
