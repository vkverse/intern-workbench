...
id: TEST-006
phase: TEST
title: Test APIs, Components, and Forms
type: BUILD
...
Complete a reviewable engineering exercise that demonstrates **test apis, components, and forms** and explain the decisions you made.

## Context

This is task 6 in the Testing phase. It builds one durable skill inside the same GitHub workflow used for product work: issue, branch, implementation, validation, pull request, review, and revision.

## What You Need to Learn

- The core concepts behind test apis, components, and forms.
...
1. Create or update the task artifact under `projects/intermediate/test-006/`; do not scatter unrelated changes across the repository.
2. Add a short `README.md` in that task artifact containing the problem, your plan, commands used, and result.
3. Produce the smallest working example or analysis that demonstrates test apis, components, and forms.
...
- Primary workspace: `projects/intermediate/test-006/`
- Evidence: `projects/intermediate/test-006/README.md`
...
- [ ] The artifact directly demonstrates test apis, components, and forms.
...
From the repository root, enter `projects/intermediate/test-006` and follow its README. If the task is documentation or investigation only, render/preview the Markdown and run the repository validation command instead.
...
1. Confirm the assigned issue title starts with `[TEST-006]`.
...
`build: complete test-006 test apis, components, and forms`
