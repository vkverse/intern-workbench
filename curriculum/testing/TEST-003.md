...
Complete a reviewable engineering exercise that demonstrates **write integration tests** and explain the decisions you made.

## Context

This is task 3 in the Testing phase. It builds one durable skill inside the same GitHub workflow used for product work: issue, branch, implementation, validation, pull request, review, and revision.

## What You Need to Learn

- The core concepts behind write integration tests.
...
1. Create or update the task artifact under `projects/intermediate/test-003/`; do not scatter unrelated changes across the repository.
2. Add a short `README.md` in that task artifact containing the problem, your plan, commands used, and result.
3. Produce the smallest working example or analysis that demonstrates write integration tests.
...
- Primary workspace: `projects/intermediate/test-003/`
- Evidence: `projects/intermediate/test-003/README.md`
...
- [ ] The artifact directly demonstrates write integration tests.
...
From the repository root, enter `projects/intermediate/test-003` and follow its README. If the task is documentation or investigation only, render/preview the Markdown and run the repository validation command instead.
...
1. Confirm the assigned issue title starts with `[TEST-003]`.
...
`task/<github-username>/test-003-write-integration-tests`

## Commit Message

`build: complete test-003 write integration tests`
