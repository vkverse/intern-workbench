...
id: TEST-005
phase: TEST
title: Mock External Boundaries Carefully
type: BUILD
...
# TEST-005 — Mock External Boundaries Carefully

## Objective

Complete a reviewable engineering exercise that demonstrates **mock external boundaries carefully** and explain the decisions you made.

## Context

This is task 5 in the Testing phase. It builds one durable skill inside the same GitHub workflow used for product work: issue, branch, implementation, validation, pull request, review, and revision.

## What You Need to Learn

- The core concepts behind mock external boundaries carefully.
...
1. Create or update the task artifact under `projects/intermediate/test-005/`; do not scatter unrelated changes across the repository.
2. Add a short `README.md` in that task artifact containing the problem, your plan, commands used, and result.
3. Produce the smallest working example or analysis that demonstrates mock external boundaries carefully.
...
- Primary workspace: `projects/intermediate/test-005/`
- Evidence: `projects/intermediate/test-005/README.md`
...
- [ ] The artifact directly demonstrates mock external boundaries carefully.
...
From the repository root, enter `projects/intermediate/test-005` and follow its README. If the task is documentation or investigation only, render/preview the Markdown and run the repository validation command instead.
...
1. Confirm the assigned issue title starts with `[TEST-005]`.
...
`task/<github-username>/test-005-mock-external-boundaries-carefully`

## Commit Message

`build: complete test-005 mock external boundaries carefully`
