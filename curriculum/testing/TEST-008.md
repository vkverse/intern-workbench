...
id: TEST-008
phase: TEST
title: Repair Failing Tests and Enforce Them in CI
type: BUILD
...
# TEST-008 — Repair Failing Tests and Enforce Them in CI

## Objective

Complete a reviewable engineering exercise that demonstrates **repair failing tests and enforce them in ci** and explain the decisions you made.

## Context

This is task 8 in the Testing phase. It builds one durable skill inside the same GitHub workflow used for product work: issue, branch, implementation, validation, pull request, review, and revision.

## What You Need to Learn

- The core concepts behind repair failing tests and enforce them in ci.
...
1. Create or update the task artifact under `projects/intermediate/test-008/`; do not scatter unrelated changes across the repository.
2. Add a short `README.md` in that task artifact containing the problem, your plan, commands used, and result.
3. Produce the smallest working example or analysis that demonstrates repair failing tests and enforce them in ci.
...
- Primary workspace: `projects/intermediate/test-008/`
- Evidence: `projects/intermediate/test-008/README.md`
...
- [ ] The artifact directly demonstrates repair failing tests and enforce them in ci.
...
From the repository root, enter `projects/intermediate/test-008` and follow its README. If the task is documentation or investigation only, render/preview the Markdown and run the repository validation command instead.
...
1. Confirm the assigned issue title starts with `[TEST-008]`.
...
`task/<github-username>/test-008-repair-failing-tests-and-enforce-them-in-c`

## Commit Message

`build: complete test-008 repair failing tests and enforce them in ci`
