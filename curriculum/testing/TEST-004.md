...
Complete a reviewable engineering exercise that demonstrates **write end-to-end tests** and explain the decisions you made.

## Context

This is task 4 in the Testing phase. It builds one durable skill inside the same GitHub workflow used for product work: issue, branch, implementation, validation, pull request, review, and revision.

## What You Need to Learn

- The core concepts behind write end-to-end tests.
...
1. Create or update the task artifact under `projects/intermediate/test-004/`; do not scatter unrelated changes across the repository.
2. Add a short `README.md` in that task artifact containing the problem, your plan, commands used, and result.
3. Produce the smallest working example or analysis that demonstrates write end-to-end tests.
...
- Primary workspace: `projects/intermediate/test-004/`
- Evidence: `projects/intermediate/test-004/README.md`
...
- [ ] The artifact directly demonstrates write end-to-end tests.
...
From the repository root, enter `projects/intermediate/test-004` and follow its README. If the task is documentation or investigation only, render/preview the Markdown and run the repository validation command instead.
...
1. Confirm the assigned issue title starts with `[TEST-004]`.
...
`build: complete test-004 write end-to-end tests`
