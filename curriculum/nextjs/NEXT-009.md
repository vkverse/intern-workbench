...
id: NEXT-009
phase: NEXT
title: Route Handlers, Auth, and Database Integration
type: BUILD
...
Complete a reviewable engineering exercise that demonstrates **route handlers, auth, and database integration** and explain the decisions you made.

## Context

This is task 9 in the Next.js phase. It builds one durable skill inside the same GitHub workflow used for product work: issue, branch, implementation, validation, pull request, review, and revision.

## What You Need to Learn

- The core concepts behind route handlers, auth, and database integration.
...
1. Create or update the task artifact under `projects/full-stack/web/next-009/`; do not scatter unrelated changes across the repository.
2. Add a short `README.md` in that task artifact containing the problem, your plan, commands used, and result.
3. Produce the smallest working example or analysis that demonstrates route handlers, auth, and database integration.
...
- Primary workspace: `projects/full-stack/web/next-009/`
- Evidence: `projects/full-stack/web/next-009/README.md`
...
- [ ] The artifact directly demonstrates route handlers, auth, and database integration.
...
From the repository root, enter `projects/full-stack/web/next-009` and follow its README. If the task is documentation or investigation only, render/preview the Markdown and run the repository validation command instead.
...
1. Confirm the assigned issue title starts with `[NEXT-009]`.
...
`task/<github-username>/next-009-route-handlers-auth-and-database-integrati`

## Commit Message

`build: complete next-009 route handlers, auth, and database integration`
