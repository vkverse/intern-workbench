...
id: PERF-005
phase: PERF
title: Diagnose Database Query Performance
type: INVESTIGATE
difficulty: advanced
...
# PERF-005 — Diagnose Database Query Performance

## Objective

Complete a reviewable engineering exercise that demonstrates **diagnose database query performance** and explain the decisions you made.

## Context

This is task 5 in the Performance phase. It builds one durable skill inside the same GitHub workflow used for product work: issue, branch, implementation, validation, pull request, review, and revision.

## What You Need to Learn

- The core concepts behind diagnose database query performance.
...
1. Create or update the task artifact under `projects/advanced/performance-lab/perf-005/`; do not scatter unrelated changes across the repository.
2. Add a short `README.md` in that task artifact containing the problem, your plan, commands used, and result.
3. Produce the smallest working example or analysis that demonstrates diagnose database query performance.
...
- Primary workspace: `projects/advanced/performance-lab/perf-005/`
- Evidence: `projects/advanced/performance-lab/perf-005/README.md`
...
- [ ] The artifact directly demonstrates diagnose database query performance.
...
From the repository root, enter `projects/advanced/performance-lab/perf-005` and follow its README. If the task is documentation or investigation only, render/preview the Markdown and run the repository validation command instead.
...
1. Confirm the assigned issue title starts with `[PERF-005]`.
...
`task/<github-username>/perf-005-diagnose-database-query-performance`

## Commit Message

`investigate: complete perf-005 diagnose database query performance`
