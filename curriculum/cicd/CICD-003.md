...
id: CICD-003
phase: CICD
title: Automate Lint, Format, and Type Checks
type: BUILD
difficulty: advanced
...
Complete a reviewable engineering exercise that demonstrates **automate lint, format, and type checks** and explain the decisions you made.

## Context

This is task 3 in the CI/CD phase. It builds one durable skill inside the same GitHub workflow used for product work: issue, branch, implementation, validation, pull request, review, and revision.

## What You Need to Learn

- The core concepts behind automate lint, format, and type checks.
...
1. Create or update the task artifact under `.github/workflows/cicd-003/`; do not scatter unrelated changes across the repository.
2. Add a short `README.md` in that task artifact containing the problem, your plan, commands used, and result.
3. Produce the smallest working example or analysis that demonstrates automate lint, format, and type checks.
...
- Primary workspace: `.github/workflows/cicd-003/`
- Evidence: `.github/workflows/cicd-003/README.md`
...
- [ ] The artifact directly demonstrates automate lint, format, and type checks.
...
From the repository root, enter `.github/workflows/cicd-003` and follow its README. If the task is documentation or investigation only, render/preview the Markdown and run the repository validation command instead.
...
1. Confirm the assigned issue title starts with `[CICD-003]`.
...
`task/<github-username>/cicd-003-automate-lint-format-and-type-checks`

## Commit Message

`build: complete cicd-003 automate lint, format, and type checks`
