...
# CSS-006 — Build Layouts with Grid

## Objective

Complete a reviewable engineering exercise that demonstrates **build layouts with grid** and explain the decisions you made.

## Context

This is task 6 in the CSS phase. It builds one durable skill inside the same GitHub workflow used for product work: issue, branch, implementation, validation, pull request, review, and revision.

## What You Need to Learn

- The core concepts behind build layouts with grid.
...
1. Create or update the task artifact under `projects/beginner/site/css-006/`; do not scatter unrelated changes across the repository.
2. Add a short `README.md` in that task artifact containing the problem, your plan, commands used, and result.
3. Produce the smallest working example or analysis that demonstrates build layouts with grid.
...
- Primary workspace: `projects/beginner/site/css-006/`
- Evidence: `projects/beginner/site/css-006/README.md`
...
- [ ] The artifact directly demonstrates build layouts with grid.
...
From the repository root, enter `projects/beginner/site/css-006` and follow its README. If the task is documentation or investigation only, render/preview the Markdown and run the repository validation command instead.
...
1. Confirm the assigned issue title starts with `[CSS-006]`.
...
`task/<github-username>/css-006-build-layouts-with-grid`

## Commit Message

`build: complete css-006 build layouts with grid`
