...
id: ORI-002
phase: ORI
title: Frontend, Backend, Client, and Server
type: LEARN
difficulty: beginner
guidance: GUIDED
...
# ORI-002 — Frontend, Backend, Client, and Server

## Objective

Complete a reviewable engineering exercise that demonstrates **frontend, backend, client, and server** and explain the decisions you made.

## Context

This is task 2 in the Orientation phase. It builds one durable skill inside the same GitHub workflow used for product work: issue, branch, implementation, validation, pull request, review, and revision.

## What You Need to Learn

- The core concepts behind frontend, backend, client, and server.
...
1. Create or update the task artifact under `notes/orientation.md/ori-002/`; do not scatter unrelated changes across the repository.
2. Add a short `README.md` in that task artifact containing the problem, your plan, commands used, and result.
3. Produce the smallest working example or analysis that demonstrates frontend, backend, client, and server.
...
- Primary workspace: `notes/orientation.md/ori-002/`
- Evidence: `notes/orientation.md/ori-002/README.md`
...
- [ ] The artifact directly demonstrates frontend, backend, client, and server.
...
From the repository root, enter `notes/orientation.md/ori-002` and follow its README. If the task is documentation or investigation only, render/preview the Markdown and run the repository validation command instead.
...
1. Confirm the assigned issue title starts with `[ORI-002]`.
...
`task/<github-username>/ori-002-frontend-backend-client-and-server`

## Commit Message

`learn: complete ori-002 frontend, backend, client, and server`
