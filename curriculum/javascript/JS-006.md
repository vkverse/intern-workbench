...
id: JS-006
phase: JS
title: Select and Modify the DOM
type: BUILD
difficulty: beginner
...
# JS-006 — Select and Modify the DOM

## Objective

Complete a reviewable engineering exercise that demonstrates **select and modify the dom** and explain the decisions you made.

## Context

This is task 6 in the JavaScript phase. It builds one durable skill inside the same GitHub workflow used for product work: issue, branch, implementation, validation, pull request, review, and revision.

## What You Need to Learn

- The core concepts behind select and modify the dom.
...
1. Create or update the task artifact under `projects/intermediate/browser-app/js-006/`; do not scatter unrelated changes across the repository.
2. Add a short `README.md` in that task artifact containing the problem, your plan, commands used, and result.
3. Produce the smallest working example or analysis that demonstrates select and modify the dom.
...
- Primary workspace: `projects/intermediate/browser-app/js-006/`
- Evidence: `projects/intermediate/browser-app/js-006/README.md`
...
- [ ] The artifact directly demonstrates select and modify the dom.
...
From the repository root, enter `projects/intermediate/browser-app/js-006` and follow its README. If the task is documentation or investigation only, render/preview the Markdown and run the repository validation command instead.
...
1. Confirm the assigned issue title starts with `[JS-006]`.
...
`task/<github-username>/js-006-select-and-modify-the-dom`

## Commit Message

`build: complete js-006 select and modify the dom`
