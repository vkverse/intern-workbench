...
id: JS-010
phase: JS
title: Promises, Fetch, and Async Await
type: BUILD
difficulty: beginner
guidance: GUIDED
...
# JS-010 — Promises, Fetch, and Async Await
...
This v0.5 exercise is part of the [starter route](../../tracks/starter.md). Read the [exercise contract](../../exercises/fetch/README.md) before editing. Your mentor agrees the assignment and AI policy in an Issue.
...
Run `npm run workbench -- start JS-010 octocat` from the repository root after replacing `octocat` with your GitHub username. Work in `submissions/<github-username>/JS-010/`. Keep shared curriculum and exercise contracts unchanged.
...
`task/<github-username>/js-010-fetch`

Replace the placeholder; for example `task/octocat/js-010-fetch`.

## Commit Message

`docs: submit JS-010 evidence` for documentation; use `feat:` or `fix:` when implementing behavior.
...
- [ ] Verify: Successful, empty, HTTP-error, malformed JSON, and network-error cases behave as specified.
- [ ] Verify: Tests use a fake fetcher, so no live endpoint or token is needed.
- [ ] Verify: README distinguishes HTTP failures from rejected network promises.
