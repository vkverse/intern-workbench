...
id: JS-005
phase: JS
title: Modules, JSON, and Error Handling
type: BUILD
difficulty: beginner
...
# JS-005 — Modules, JSON, and Error Handling

## Objective

Complete a reviewable engineering exercise that demonstrates **modules, json, and error handling** and explain the decisions you made.

## Context

This is task 5 in the JavaScript phase. It builds one durable skill inside the same GitHub workflow used for product work: issue, branch, implementation, validation, pull request, review, and revision.

## What You Need to Learn

- The core concepts behind modules, json, and error handling.
...
1. Create or update the task artifact under `projects/intermediate/browser-app/js-005/`; do not scatter unrelated changes across the repository.
2. Add a short `README.md` in that task artifact containing the problem, your plan, commands used, and result.
3. Produce the smallest working example or analysis that demonstrates modules, json, and error handling.
...
- Primary workspace: `projects/intermediate/browser-app/js-005/`
- Evidence: `projects/intermediate/browser-app/js-005/README.md`
...
- [ ] The artifact directly demonstrates modules, json, and error handling.
...
From the repository root, enter `projects/intermediate/browser-app/js-005` and follow its README. If the task is documentation or investigation only, render/preview the Markdown and run the repository validation command instead.
...
1. Confirm the assigned issue title starts with `[JS-005]`.
...
`task/<github-username>/js-005-modules-json-and-error-handling`

## Commit Message

`build: complete js-005 modules, json, and error handling`
