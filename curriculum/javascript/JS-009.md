...
# JS-009 — Browser Storage

## Objective

Complete a reviewable engineering exercise that demonstrates **browser storage** and explain the decisions you made.

## Context

This is task 9 in the JavaScript phase. It builds one durable skill inside the same GitHub workflow used for product work: issue, branch, implementation, validation, pull request, review, and revision.

## What You Need to Learn

- The core concepts behind browser storage.
...
1. Create or update the task artifact under `projects/intermediate/browser-app/js-009/`; do not scatter unrelated changes across the repository.
2. Add a short `README.md` in that task artifact containing the problem, your plan, commands used, and result.
3. Produce the smallest working example or analysis that demonstrates browser storage.
...
- Primary workspace: `projects/intermediate/browser-app/js-009/`
- Evidence: `projects/intermediate/browser-app/js-009/README.md`
...
- [ ] The artifact directly demonstrates browser storage.
...
From the repository root, enter `projects/intermediate/browser-app/js-009` and follow its README. If the task is documentation or investigation only, render/preview the Markdown and run the repository validation command instead.
...
1. Confirm the assigned issue title starts with `[JS-009]`.
...
`task/<github-username>/js-009-browser-storage`

## Commit Message

`build: complete js-009 browser storage`
